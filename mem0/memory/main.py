...
def _setup_file_logger():
    """Ensure errors are also written to /root/ljz/mymem/evaluation/logs/mem0.log."""
    log_dir = "/root/ljz/mymem/evaluation/logs"
    os.makedirs(log_dir, exist_ok=True)
    log_path = os.path.join(log_dir, "mem0.log")

    # Avoid重复添加相同的文件 handler
...
class Memory(MemoryBase):
    ...
    def _add_to_vector_store(self, messages, metadata, filters, infer):
                ...
                try:
                    ...
                    if not extracted_json.strip():
                        ...
                        try:
                            ...
                            logger.warning(f"Failed to parse extracted JSON: {extracted_json[:100]}")
                            new_retrieved_facts = []
        except Exception as e:
            # 在 JSON 解析等任意异常时，把原始响应也打出来，方便排查
            logger.error(f"Error in new_retrieved_facts: {e}")
            try:
                logger.error(f"Raw response when error occurred: {response[:500]}")
            except Exception:
                logger.error(f"Raw response is not a string: {type(response)}")
            new_retrieved_facts = []
        ...
        for new_mem in new_retrieved_facts:
            ...
            # NOTE: we need to fetch足够多的旧记忆以覆盖 LLM 返回的索引，否则会出现 id 不存在的报错
            existing_memories = self.vector_store.search(
                query=new_mem,
                vectors=messages_embeddings,
                limit=10,  # 原来为 5，导致当 LLM 返回更高索引（如 6、7）时 temp_uuid_mapping 不包含对应 id
        ...
        if new_retrieved_facts:
            ...
            try:
                if not response or not response.strip():
                    ...
                    response = remove_code_blocks(response)
                    new_memories_with_actions = json_repair.loads(response)
            except Exception as e:
                raw = (response or "").strip()
                snippet = raw[:2000]
                logger.error(f"Invalid JSON response: {e}. Raw snippet: {snippet}")
                new_memories_with_actions = {}
        else:
            new_memories_with_actions = {}
        ...
        try:
            for resp in new_memories_with_actions.get("memory", []):
                ...
                try:
                    ...
                    # 兜底 1：过滤掉 UPDATE 但 text 和 old_memory 完全相同的条目
                    # 这种情况下本质上是「无变化」，继续 UPDATE 只会徒增噪声，甚至可能因为 id 问题导致异常。
                    if event_type == "UPDATE":
                        old_memory_text = resp.get("old_memory")
                        try:
                            # 做一次安全的 strip，避免因为前后空格 / 换行不同导致误判
                            normalized_new = (action_text or "").strip()
                            normalized_old = (old_memory_text or "").strip()
                        except Exception:
                            normalized_new = action_text
                            normalized_old = old_memory_text

                        if normalized_new and normalized_new == normalized_old:
                            logger.info(
                                "Skipping UPDATE entry because `text` is identical to `old_memory`. "
                                f"id={resp.get('id')}, text={repr(action_text)}"
                            )
                            # 等价于让这一条变成 NONE：既不更新内容，也不触发 id 映射
                            continue
                    ...
                    if event_type == "ADD":
                        ...
                        mem_key = resp.get("id")
                        if mem_key is None or str(mem_key) not in temp_uuid_mapping:
                            # 兜底 2：如果 UPDATE 的 id 非法，但提供了 old_memory，就尝试通过 old_memory 文本
                            # 在当前 retrieved_old_memory 里找到对应的记忆，并将这次 UPDATE 重定向到那条记忆。
                            old_memory_text = resp.get("old_memory")
                            remapped = False
                            if old_memory_text:
                                try:
                                ...
                                if normalized_old:
                                    for item in retrieved_old_memory:
                                        try:
                                            item_text_norm = (item.get("text") or "").strip()
                                        except Exception:
                                            item_text_norm = item.get("text")

                                        if item_text_norm and item_text_norm == normalized_old:
                                            # 这里的 item["id"] 已被替换成临时整数索引字符串
                                            alt_key = str(item.get("id"))
                                            if alt_key in temp_uuid_mapping:
                                                mem_id = temp_uuid_mapping[alt_key]
                                                logger.warning(
                                                    "Invalid UPDATE id from LLM, remapped by `old_memory`. "
                                                    f"orig_id={mem_key}, remapped_id={alt_key}, "
                                                    f"available={list(temp_uuid_mapping.keys())}. "
                                                    f"prompt={function_calling_prompt or ''} "
                            ...
                            if remapped:
                                # 已经通过 old_memory 找到对应对象并完成 UPDATE，直接处理下一条。
                                continue

                            # 如果通过 old_memory 也找不到匹配项，则保留原来的兜底逻辑：跳过该 UPDATE，避免抛异常。
                            logger.warning(
                                "Invalid UPDATE id from LLM, skipping entry. "
                                f"id={mem_key}, available={list(temp_uuid_mapping.keys())}. "
                                f"prompt={function_calling_prompt or ''} "
                                f"response={response or ''}"
                            )
                            continue

                        mem_id = temp_uuid_mapping[str(mem_key)]
                        self._update_memory(
                            memory_id=mem_id,
                            data=action_text,
                            existing_embeddings=new_message_embeddings,
                        ...
                        mem_key = resp.get("id")
                        if mem_key is None or str(mem_key) not in temp_uuid_mapping:
                            # 同理，对非法 DELETE id 也做兜底处理，避免抛异常。
                            logger.warning(
                                "Invalid DELETE id from LLM, skipping entry. "
                                f"id={mem_key}, available={list(temp_uuid_mapping.keys())}. "
                                f"prompt={function_calling_prompt or ''} "
                                f"response={response or ''}"
                            )
            ...
            for resp in new_memories_with_actions.get("memory", []):
                ...
                try:
                    ...
                    if event_type == "ADD":
                        ...
                        mem_key = resp.get("id")
                        if mem_key is None or str(mem_key) not in temp_uuid_mapping:
                            # Async 版本同样做兜底处理：非法 UPDATE id 仅告警并跳过。
                            logger.warning(
                                "Invalid UPDATE id from LLM (async), skipping entry. "
                                f"id={mem_key}, available={list(temp_uuid_mapping.keys())}. "
                                f"prompt={function_calling_prompt or ''} "
                                f"response={response or ''}"
                            )
                        ...
                        if mem_key is None or str(mem_key) not in temp_uuid_mapping:
                            logger.warning(
                                "Invalid DELETE id from LLM (async), skipping entry. "
                                f"id={mem_key}, available={list(temp_uuid_mapping.keys())}. "
                                f"prompt={function_calling_prompt or ''} "
                                f"response={response or ''}"
                            )
